...
module.exports = function () {
  ...
  return html`
    <div>
      <h1 class="horizontal-rule">Development Team</h1>
      <div class="${flexClass} w-100">
        ${teamInfo.current.map((person) => {
          return personBox(person)
        })}
      </div>
      <h1 class="horizontal-rule">Advisors</h1>
      <div class="${flexClass} w-100">
        ${teamInfo.advisors.map((person) => {
          return personBox(person)
        })}
      </div>
      <h1 class="horizontal-rule">Alumni</h1>
      <div class="${flexClass} w-100">
    ...
    return html`
      <div class="inline-flex w-100 w-third-ns pa3">
        <div class="bg-white w-100 pa3 pa4-ns b--dat-blue ba bw2">
          <div class="tc">
            <a href="http://github.com/${info.github}"><img src="${avatar(info.github)}" class="br-100 h3 w3 dib" title="${info.name}"></a>
            <h1 class="f4">${info.name}</h1>
